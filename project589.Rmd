...
```{r}
fit <- ppm(hornet_ppp ~ elev + I(elev^2) + fors + I(fors^2) + dist_water + I(dist_water^2), data = DATA)

fit

```
From the Ztest, we'd better remove fors^2
```{r}
fit_re <- ppm(hornet_ppp ~ elev + I(elev^2) + fors + dist_water + I(dist_water^2), data = DATA)
...
quadrat.test(fit_re, nx = 10, ny = 10)
```
...
par_res_elev0 <- parres(fit_re, "elev")

#Calculate the relative intensity as a function of for
par_res_forst0 <- parres(fit_re, "fors")

par_res_water0 <- parres(fit_re,"dist_water")
...
```
The plots show the model cannot fit the data well.

```{r}
library(splines)

#Fit the PPP model
fit_smooth <- ppm(hornet_ppp ~ bs(elev,7) + bs(fors, 7) + bs(dist_water,7), data = DATA, use.gam = TRUE)
...
#Calculate the relative intensity as a function of for
par_res_forst <- parres(fit_smooth, "fors")
...
```{r}
AIC(fit_re);AIC(fit_smooth)
```

```{r}
BIC(fit_re);BIC(fit_smooth)
```

```{r}
anova(fit_re, fit_smooth, test = "LRT")
```
